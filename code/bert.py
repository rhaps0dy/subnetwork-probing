...
def unpack(ann, sent_lengths):
    ...
    assert sum(sent_lengths) == packed_len, "Packed len is {} but should be {}".format(
        sent_lengths, packed_len
    )
...
class WordLevelBert(nn.Module):
    ...
    def forward(self, sentences, padded=True, include_clssep=True):
        return self.annotate(
            sentences, output_hidden=False, padded=padded, include_clssep=include_clssep
        )[0]

    def annotate(
        self, sentences, output_hidden=False, padded=True, include_clssep=True
    ):
        """
        ...
        for s_num, sentence in enumerate(sentences):
            ...
            for word in sentence:
                ...
                assert len(word_tokens) > 0, "Unknown word: {} in {}".format(
                    word, sentence
                )
        ...
        if output_hidden:
            features, _, hidden, attention = self.bert(
    ...
    def reset_weights(self, encoder_only=True):
        for name, module in self.named_modules():
            if hasattr(module, "reset_parameters") and (
                "encoder" in name or not encoder_only
            ):

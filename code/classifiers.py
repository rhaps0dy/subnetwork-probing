...
class UDModel(nn.Module):
    def __init__(self, bert_encoder, pos_vocab_size, arc_vocab_size):
        ...
        self.mlp_head = nn.Sequential(
            nn.Linear(d_features, d_label_hidden), nn.ReLU(), nn.Dropout(p=0.33)  # 1024
        )
        ...
        self.W_arc = nn.Parameter(
            torch.Tensor(arc_vocab_size, d_label_hidden_arc, d_label_hidden_arc)
        )
        self.head_vec_arc = nn.Parameter(
            torch.Tensor(d_label_hidden_arc, arc_vocab_size)
        )
        self.dep_vec_arc = nn.Parameter(
            torch.Tensor(d_label_hidden_arc, arc_vocab_size)
        )
    ...
    def predict_batch(self, sentences, correct_heads):
        ...
        for i in range(num_sent):
            ...
            if ann_head_arc_perdep is None:
                ...
                ann_head_arc_perdep = torch.cat(
                    (ann_head_arc_perdep, ann_head_arc_perdep_sent), dim=0
                )
        ...
        ann_head_arc = ann_head_arc.unsqueeze(0).repeat(self.arc_vocab_size, 1, 1, 1)
        vocab = self.arc_vocab_size
        ann_head_arc = ann_head_arc.reshape(vocab, num_sent * pad_len, dim)
        # (num_sent * pad_len) x vocab x 256
        ann_head_arc = (
            torch.bmm(ann_head_arc, self.W_arc)
            .reshape(vocab, num_sent, pad_len, dim)
            .permute(1, 2, 0, 3)
            .reshape(num_sent * pad_len, vocab, dim)
        )

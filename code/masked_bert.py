...
class MaskedWordLevelBert(WordLevelBert):
    ...
    def get_sparsity(self, layer, head):  # both are 0-indexed
        ...
        with torch.no_grad():
            ...
            value_mask = layer.value.produce_mask_reshaped().reshape(
                num_heads, dim // num_heads, dim
            )[head]

            if hasattr(layer.query, "produce_mask_reshaped"):
                query_mask = layer.query.produce_mask_reshaped().reshape(
                    num_heads, dim // num_heads, dim
                )[head]
                key_mask = layer.key.produce_mask_reshaped().reshape(
                    num_heads, dim // num_heads, dim
                )[head]
            else:
    ...
    def get_sparsity_layer(self, layer):  # both are 0-indexed
        ...
        with torch.no_grad():
            ...
            sparsity = (
                ...
                + torch.sum(mask2)
                + torch.sum(mask3)
                + torch.sum(value_mask)
                + torch.sum(query_mask)
                + torch.sum(key_mask)
            )
            sparsity = sparsity / (
                mask1.numel()
    ...
    def get_sparsity_layer_attn(self, layer):  # both are 0-indexed
        ...
        with torch.no_grad():
            ...
            sparsity = (
                torch.sum(value_mask) + torch.sum(query_mask) + torch.sum(key_mask)
            )
            sparsity = sparsity / (
                value_mask.numel() + query_mask.numel() + key_mask.numel()
            )
